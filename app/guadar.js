...
function existeDuplicado(nombre,numero,ignoreKey=null){
  return Object.keys(contactosGlobal).some(key=>{
    ...
    return nom === nombre.toLowerCase() && c.numero === numero;
  });
}
...
function renderLista(data){
  ...
  Object.keys(data || {}).forEach(key=>{
    ...
    if(
      ...
      lista.innerHTML += `
      <div class="card">
        <strong><i class="fa-solid fa-user"></i> ${c.nombre || "Sin nombre"}</strong><br>
        <i class="fa-solid fa-phone"></i> ${numero}<br>
        <i class="fa-solid fa-wallet"></i> ${c.destino || "Yape"}
        <div class="actions">
